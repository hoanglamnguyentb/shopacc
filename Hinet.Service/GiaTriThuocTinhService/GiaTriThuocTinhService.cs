...
namespace Hinet.Service.GiaTriThuocTinhService
{
	...
	ILog _loger;
        ...
        public PageListResultBO<GiaTriThuocTinhDto> GetDaTaByPage(GiaTriThuocTinhSearchDto searchModel, int pageIndex = 1, int pageSize = 20)
        {
            var query = from GiaTriThuocTinhtbl in _GiaTriThuocTinhRepository.GetAllAsQueryable()
                        ...
                        select new GiaTriThuocTinhDto
                        {
							...
							GiaTriTxt = GiaTriThuocTinhtbl.GiaTriTxt,
		...
		{
			query = query.Where(x => x.GiaTriTxt.Contains(searchModel.GiaTriTextFilter));
		}
        ...
        public void DeleteByTaiKhoanId(int taiKhoanId)
        {
            var query = _GiaTriThuocTinhRepository.GetQueryable().Where(x => x.TaiKhoanId == taiKhoanId);
            _GiaTriThuocTinhRepository.DeleteRange(query);
            _GiaTriThuocTinhRepository.Save();
        }
