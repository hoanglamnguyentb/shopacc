...
namespace Hinet.Service.GameService
{
    public class GameService : EntityService<Game>, IGameService
    {
            ...
            var query = from game in _GameRepository.GetQueryable()
                        ...
                        select new GameDto
                        {
                            ...
                            ListDanhMucGame = danhMucGameGrp.Select(dm => new DanhMucGameDto
                            {
                                ...
                                LaLoaiDongGia = dm.LaLoaiDongGia,
                                MoTa = dm.MoTa,
                                Slug = dm.Slug,
                                GiaGoc = dm.GiaGoc,
                                GiaKhuyenMai = dm.GiaKhuyenMai,
                                SoLuongTaiKhoan = taiKhoanQuery.Count(x => x.DanhMucGameId == dm.Id 
                                    && x.TrangThai != TrangThaiTaiKhoanConstant.DABAN)
                            }).ToList()
                        };
        ...
        public PageListResultBO<TaiKhoanDto> GetTaiKhoanPagedByDanhMucSlug(string slug, TaiKhoanSearchDto search, int pageIndex, int pageSize)
        {
            ...
            var query = from tk in _taiKhoanRepository.GetQueryable().Where(x => x.TrangThai == TrangThaiTaiKhoanConstant.CHUABAN)
                        ...
                        select new TaiKhoanDto
                        {
                            ...
                            Mota = tk.Mota,
                            ViTri = tk.ViTri,
                            AnhBia = tlQuery.Where(tl => tl.Item_ID == tk.Id)
                                        .Select(tl => tl.DuongDanFile)
                                        .FirstOrDefault(),
        ...
        public List<DanhMucGameDto> GetListDanhMucGameBySlug(string gameSlug = null)
        {
            var tkQuery = _taiKhoanRepository.GetQueryable();
            ...
            var query = from g in _GameRepository.GetQueryable()
                        ...
                        select new DanhMucGameDto
                        {
                            ...
                            GiaKhuyenMai = dm.GiaKhuyenMai,
                            LaLoaiDongGia = dm.LaLoaiDongGia,
                            SoLuongTaiKhoan = (from  tk in tkQuery
                                               where tk.DanhMucGameId == dm.Id
                                               select tk.Id).Count()
                        };
            if(!string.IsNullOrEmpty(gameSlug))
            {
                query = query.Where(x => x.GameSlug == gameSlug);
            }
        ...
        public List<TaiKhoanDto> GetListTaiKhoanDaXem(List<long> daXemIds)
        {
            ...
            var gtttQuery = _giaTriThuocTinhRepository.GetAllAsQueryable();
            var tlQuery = _taiLieuDinhKemRepository.GetQueryable();
            var query = from tk in _taiKhoanRepository.GetQueryable().Where(x => daXemIds.Contains(x.Id))
                        join gttt in gtttQuery 
                        on tk.Id equals gttt.TaiKhoanId into gtttGrp
                        ...
                        select new TaiKhoanDto
                        {
                            ...
                            Mota = tk.Mota,
                            ViTri = tk.ViTri,
                            AnhBia = tl != null ? tl.DuongDanFile : null,
                            ListGTTT = gtttGrp.ToList(),
                            DanhMucGame = danhMucGame,
                            Game = game,
                        };
        ...
        public List<TaiKhoanDto> GetListTaiKhoanLienQuan(long id)
        {
            var taiKhoan = _taiKhoanRepository.GetQueryable()
            ...
            var query = from tk in tkQuery
                        where tk.Id != id && tk.DanhMucGameId == taiKhoan.DanhMucGameId
