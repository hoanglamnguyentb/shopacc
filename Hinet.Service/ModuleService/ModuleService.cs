...
namespace Hinet.Service.ModuleService
{
    public class ModuleService : EntityService<Module>, IModuleService
    {
        ...
        public PageListResultBO<ModuleDTO> GetDataByPage(ModuleSearchDTO searchParams, int pageIndex = 1, int pageSize = 20)
        {
            var queryResult = (from module in this._moduleRepository.GetAllAsQueryable()
                               ...
                               select new ModuleDTO()
                               {
                                   ...
                                   AllowFilterScope = module.AllowFilterScope,
        ...
        /// <summary>
        /// Lấy danh sách chức năng giới hạn dữ liệu
        /// </summary>
        /// <returns></returns>
        public List<Module> GetListModuleLimitData()
        {
            return _moduleRepository.GetAllAsQueryable().Where(x => x.AllowFilterScope == true).ToList();
        }
    }
}