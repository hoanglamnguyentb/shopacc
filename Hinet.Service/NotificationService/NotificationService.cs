...
namespace Hinet.Service.NotificationService
{
    public class NotificationService : EntityService<Notification>, INotificationService
    {
        ...
        public int GetUnreadNotificationCount(long currentUserId)
        {
            var count = _NotificationRepository.GetAllAsQueryable()
                .Count(x => x.ToUser == currentUserId && x.IsRead == false);
            return count;
        }
