...
{
    [Authorize]
    public class ManageController : EndUserController
    {
        ...
        public ActionResult EndUserChangePassword()
        {
            return View();
        }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        [CaptchaValidationActionFilter("CaptchaCode", "ChangePasswordUserCaptcha", "Sai mã xác nhận!")]
        public async Task<ActionResult> EndUserChangePassword(ChangePasswordViewModel model)
        {
            if (!ModelState.IsValid)
            {
                return View(model);
            ...
            if (result.Succeeded)
            {
                ...
                ViewBag.Temp = "Thay Đổi mật khẩu thành công";
                TempData["Msg"] = 1;
                MvcCaptcha.ResetCaptcha("ChangePasswordUserCaptcha");
                return View();
            }
            AddErrors(result);
            return View(model);
