            ...
            if (filterContext.HttpContext.Session != null)
            {
               ...
               || filterContext.ActionDescriptor.ControllerDescriptor.IsDefined(typeof(AllowAnonymousAttribute), true);
                if (!skipAuthorization)
                {
                    ...
                    //    var logChucNang = new CoreLogChucNang();
                    //    logChucNang.Action = action;
                    //    logChucNang.Controller = controller;
                    //    logChucNang.Area = area.ToLower();
                    //    var url = string.Format("/{0}/{1}/{2}", area, controller, action);
                    //    logChucNang.Url = url.ToLower();
                    //    _coreLogChucNangService.Save(logChucNang);
                    //}
