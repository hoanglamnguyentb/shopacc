...
namespace Hinet.API2.Common
{
    public class TokenJWTProvider
    {
        ...
        public static Task<ClaimsIdentity> CreateClaimsIdentitiesAsync(UserDto user)
        {
            ClaimsIdentity claimsIdentity = new ClaimsIdentity();
            claimsIdentity.AddClaim(new Claim(ClaimTypes.NameIdentifier, user.Id.ToString()));
            claimsIdentity.AddClaim(new Claim(ClaimTypes.Name, user.UserName));
            claimsIdentity.AddClaim(new Claim(ClaimTypes.Role, string.Join(";", user.ListRoles.Select(x => x.Code))));
            var test = string.Join(";", user.ListRoles);
